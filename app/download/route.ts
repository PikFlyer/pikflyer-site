...
const DOWNLOAD_FILES = new Set([
  "pikflyer-xiaochibang-android-v2.0.6.apk",
]);

function getDownloadLocation(request: Request): Response | string {
  const url = new URL(request.url);
  const file = url.searchParams.get("file") || "pikflyer-xiaochibang-android-v2.0.6.apk";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    await recordSiteStat(request, "download", "pikflyer-xiaochibang-android-v2.0.6.apk");
