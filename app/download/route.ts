...
const DOWNLOAD_FILES = new Set([
  "pikflyer-xiaochibang-android-v1.0.14.apk",
]);

function getDownloadLocation(request: Request): Response | string {
  const url = new URL(request.url);
  const file = url.searchParams.get("file") || "pikflyer-xiaochibang-android-v1.0.14.apk";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    await recordSiteStat(request, "download", "pikflyer-xiaochibang-android-v1.0.14.apk");
