...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const auth = await authorizePoiPrefetch(request, body, requestedCount);
    ...
    const granted = Math.max(1, Math.min(Number(authBody.granted || requestedCount), requestedCount));
